...
class DQNAgent:
    ...
    def act(self, state):
        ...
        if np.random.uniform(0.0, 1.0) <= self.epsilon:
            action = torch.tensor([np.random.uniform(0.0, 1.0), np.random.uniform(-0.5, 0.5)])
            return torch.round(action * 10) / 10
        else:
            action = self.model(state)
            action = ((np.exp(action.clone().detach()) - np.exp(-action.clone().detach())) / (np.exp(action.clone().detach()) + np.exp(-action.clone().detach())))
            return (torch.round(action.clone().detach() * 10) / 10)[0]
    ...
    def replay(self):
        ...
        for i in range(self.batch_size):
            if done[i]:
                target[i] = torch.tensor([reward[i], reward[i]])
            else:
                target[i] = reward[i] + self.gamma * (target_next[i])
        ...
        try:
            for e in range(self.EPISODES):
                ...
                while not done:
                    ...
                    # print('5')
                    print(colored('Step {0}: "{1}" -> reward {2}, epsilon: {3}'.format(i, action,
