...
class RopodNavDiscreteEnv(RopodEnv):
    ...
    def step(self, action: Tuple[float, float]) -> Tuple[Tuple[float, float, float],
    ...
    def get_reward(self, action: Tuple[float, float]) -> float:
        ...
        * c_2 is the vlaue of self.direction_change_punishment
        ...
        direction_change = 1 if action[1] != self.previous_action[1] else 0
        reward = (self.area * 4.0 * math.log(goal_dist + .1, .1) + 125) + \
